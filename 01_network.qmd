...
Manually remove some of the edges due to misclassification of MCL/Orthofinder.
Those edges are all of type "mcscanx", i.e. they are derived from MCScanX 
software using the Orthofinder input. The corresonding nodes are all classified 
as BAHD.
...
After removal, recalculate the communities using the following algorithms:
 ...
 - fastgreedy.community (fc),
 - walktrap.community (wc),
 - leading.eigenvector.community (le),
 - label.propagation.community (lp),
 - multilevel.community (ml),
 - infomap.community (im)


Create a `membership_community` object to store the results in.
```{r community_detection, cache = TRUE}
## https://stackoverflow.com/questions/9471906/what-are-the-differences-between-community-detection-algorithms-in-igraph
fc <- fastgreedy.community(net_cut, modularity = TRUE, 
    weights = E(net_cut)$weight)
wc <- walktrap.community(net_cut, modularity = TRUE, steps = 15, 
    weights = E(net_cut)$weight)
le <- leading.eigenvector.community(net_cut, steps = 15, 
    weights = E(net_cut)$weight)
lp <- label.propagation.community(net_cut, weights = E(net_cut)$weight)
ml <- multilevel.community(net_cut, weights = E(net_cut)$weight)
im <- infomap.community(net_cut, e.weights = E(net_cut)$weight, 
...
if (!all(names(membership(fc)) == names(membership(im))))
    stop()
membership_community <- data.frame(X = names(membership(fc)), fc = membership(fc), 
    wc = membership(wc), le = membership(le), 
...
membership_community$family <- strsplit(membership_community$family, split = "/") |> 
    ...
    unlist()
write.table(membership_community, file = "membership_community_gene_cluster_added.txt", 
...
plot_topology_parameter <- function(.topology_bahd = topology_bahd, 
    ...
    gg <- topology_l |>
        ggplot(aes(x = number_nodes, y = .data[[plot]], color = family)) +
            geom_point() +
            suppressWarnings(ggplot2::geom_point(ggplot2::aes(text = component)))
    if (.model)
        gg <- gg +
    ...
    if (log10_x)
        gg <- gg + scale_x_continuous(
            labels = function(x) format_format() (round(10^x - offset_x, digits = .digits_x)))
    if (log10_y)
        gg <- gg + scale_y_continuous(
            labels = function(x) format_format() (round(10^x - offset_y, digits = .digits_y)))
    ...
    if (.model) {
        ...
        n_rep <- min(nrow(scpl_data), nrow(bahd_data))
        print(paste("The function will perform", n_rep, "bootstrap repetitions.", sep = " "))
        
        ## bahd
        bahd_coefs <- lapply(seq_len(n_rep), function(i) {
            new_rows <- sample(x = seq_len(nrow(bahd_data)), size = nrow(bahd_data), replace = TRUE)
            new_data <- bahd_data[new_rows, ]
            .model <- lm(new_data[[plot]] ~ number_nodes, data = new_data)
            list(.model$coefficients[["(Intercept)"]], .model$coefficients[["number_nodes"]]) 
        })
        ...
        ## scpl
        scpl_coefs <- lapply(seq_len(n_rep), function(i) {
            new_rows <- sample(x = seq_len(nrow(scpl_data)), size = nrow(scpl_data), replace = TRUE)
        ...
        scpl_coefs_nn <- lapply(scpl_coefs, "[", 2) |> 
            unlist()
        df <- rbind(
            data.frame(value = bahd_coefs_intercept, family = "BAHD", coefficient = "intercept"),
            data.frame(value = bahd_coefs_nn, family = "BAHD", coefficient = "number_nodes"),
            data.frame(value = scpl_coefs_intercept, family = "SCPL", coefficient = "intercept"),
            data.frame(value = scpl_coefs_nn, family = "SCPL", coefficient = "number_nodes"))
       
        
        gg_boxplot <- ggboxplot(df, x = "family", y = "value", title = plot, 
                facet.by = "coefficient", scales = "free_y") +
            stat_compare_means(comparisons = list(c("BAHD", "SCPL")), method = "t.test")
        
        ttest_intercept <- t.test(bahd_coefs_intercept, scpl_coefs_intercept)
        ttest_nn <- t.test(bahd_coefs_nn, scpl_coefs_nn)
        
        df_ttest <- data.frame(intercept_t = ttest_intercept$statistic, 
...
```{r average_local_efficiency, warning=FALSE, message=FALSE}
## average_local_efficiency
tp <- plot_topology_parameter(plot = "average_local_efficiency", log10_x = TRUE, log10_y = FALSE,
...
```{r average_path_length, warning=FALSE, message=FALSE}
## average_path_length
tp <- plot_topology_parameter(plot = "average_path_length", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_average_path_length <- tp$ttest
```

### betweenness_normalized

```{r betweenness_normalized, warning=FALSE, message=FALSE}
## betweenness_normalized
tp <- plot_topology_parameter(plot = "betweenness_normalized_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "betweenness_normalized_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "betweenness_normalized_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "betweenness_notnormalized_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "betweenness_notnormalized_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 1, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "betweenness_notnormalized_max", log10_x = TRUE, log10_y = TRUE,
...
## closeness
tp <- plot_topology_parameter(plot = "closeness_normalized_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE, .digits_y = 4)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_closeness_normalized_min <- tp$ttest
tp <- plot_topology_parameter(plot = "closeness_normalized_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE, .digits_y = 4)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_closeness_normalized_median <- tp$ttest
tp <- plot_topology_parameter(plot = "closeness_normalized_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE, .digits_y = 3)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_closeness_normalized_max <- tp$ttest
tp <- plot_topology_parameter(plot = "closeness_notnormalized_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE, .digits_y = 6)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_closeness_notnormalized_min <- tp$ttest
tp <- plot_topology_parameter(plot = "closeness_notnormalized_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE, .digits_y = 6)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_closeness_notnormalized_median <- tp$ttest
tp <- plot_topology_parameter(plot = "closeness_notnormalized_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE, .digits_y = 6)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r constraint, warning=FALSE, message=FALSE}
## constraint
tp <- plot_topology_parameter(plot = "constraint_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_constraint_min <- tp$ttest
tp <- plot_topology_parameter(plot = "constraint_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_constraint_median <- tp$ttest
tp <- plot_topology_parameter(plot = "constraint_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
...
```{r diameter, warning=FALSE, message=FALSE}
## diameter
tp <- plot_topology_parameter(plot = "diameter", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
## degree
tp <- plot_topology_parameter(plot = "degree_normalized_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_degree_normalized_min <- tp$ttest
tp <- plot_topology_parameter(plot = "degree_normalized_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_degree_normalized_median <- tp$ttest
tp <- plot_topology_parameter(plot = "degree_normalized_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_degree_normalized_max <- tp$ttest
tp <- plot_topology_parameter(plot = "degree_notnormalized_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "degree_notnormalized_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_degree_notnormalized_median <- tp$ttest
tp <- plot_topology_parameter(plot = "degree_notnormalized_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r eccentricity, warning=FALSE, message=FALSE}
## eccentricity
tp <- plot_topology_parameter(plot = "eccentricity_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_eccentricity_min <- tp$ttest
tp <- plot_topology_parameter(plot = "eccentricity_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_eccentricity_median <- tp$ttest
tp <- plot_topology_parameter(plot = "eccentricity_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r edge_betweenness, warning=FALSE, message=FALSE}
## edge_betweenness
tp <- plot_topology_parameter(plot = "edge_betweenness_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# ttest_edge_betweenness_min <- tp$ttest
tp <- plot_topology_parameter(plot = "edge_betweenness_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# ttest_edge_betweenness_median <- tp$ttest
tp <- plot_topology_parameter(plot = "edge_betweenness_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r eigen_centrality, warning=FALSE, message=FALSE}
## eigen_centrality
tp <- plot_topology_parameter(plot = "eigen_centrality_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_eigen_centrality_min <- tp$ttest
tp <- plot_topology_parameter(plot = "eigen_centrality_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_eigen_centrality_median <- tp$ttest
tp <- plot_topology_parameter(plot = "eigen_centrality_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_eigen_centrality_max <- tp$ttest
tp <- plot_topology_parameter(plot = "eigen_centrality_value", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r global_efficiency, warning=FALSE, message=FALSE}
## global_efficiency
tp <- plot_topology_parameter(plot = "global_efficiency", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r harmonic_centrality, warning=FALSE, message=FALSE}
## harmonic_centrality
tp <- plot_topology_parameter(plot = "harmonic_centrality_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_harmonic_centrality_min <- tp$ttest
tp <- plot_topology_parameter(plot = "harmonic_centrality_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_harmonic_centrality_median <- tp$ttest
tp <- plot_topology_parameter(plot = "harmonic_centrality_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r neighborhood_size, warning=FALSE, message=FALSE}
## neighborhood_size
tp <- plot_topology_parameter(plot = "neighborhood_size_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "neighborhood_size_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_neighborhood_size_median <- tp$ttest
tp <- plot_topology_parameter(plot = "neighborhood_size_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0) 
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r radius, warning=FALSE, message=FALSE}
## radius
...
```{r strength, warning=FALSE, message=FALSE}
## strength
tp <- plot_topology_parameter(plot = "strength_min", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0, .model = TRUE)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_strength_min <- tp$ttest
tp <- plot_topology_parameter(plot = "strength_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_strength_median <- tp$ttest
tp <- plot_topology_parameter(plot = "strength_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
...
```{r shortest_paths, warning=FALSE, message=FALSE}
tp <- plot_topology_parameter(plot = "shortest_paths_min", log10_x = FALSE, log10_y = F,
    offset_x = 0, offset_y = 0, .model = FALSE)
tp$scatter
tp$scatter_plotly
# tp$bootstrap
# tp$ttest
tp <- plot_topology_parameter(plot = "shortest_paths_median", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_shortest_paths_median <- tp$ttest
tp <- plot_topology_parameter(plot = "shortest_paths_max", log10_x = TRUE, log10_y = TRUE,
    offset_x = 0, offset_y = 0)
tp$scatter
tp$scatter_plotly
tp$bootstrap
ttest_shortest_paths_max <- tp$ttest
```

### Multiple testing correction via FDR

Combine first the test results. Then adjust via FDR using the Benjamini-Hochberg
method the raw p-values using the number of models.
```{r multiple_testing_correction}
ttest_df <- rbind(
    ttest_average_path_length,
    ...
    ttest_strength_max,
    ttest_shortest_paths_median,
    ttest_shortest_paths_max)
ttest_df$intercept_p_adj <- p.adjust(ttest_df$intercept_p, method = "BH")
ttest_df$nn_p_adj <- p.adjust(ttest_df$nn_p, method = "BH")
```

Return the table with all the information (t-values, df, raw p-values, adjusted
p-values) for the intercept and the `number_nodes` parameter (gradient).
```{r}
...
What are the parameters that show differences in the 
...
```{r}
rmarkdown::paged_table(ttest_df[ttest_df$intercept_p_adj <= 0.05 & ttest_df$nn_p_adj <= 0.05, ])
```
...
greedy_clique <- function(g, n_rep = 100) {
    ...
    ## list contains the repetitions, the inner list contains the clique information
    for (i in seq_along(.components)) {
        cliques_l[[i]] <- lapply(seq_len(n_rep), 
            function(j) find_largest_non_overlapping_cliques(g = .components[[i]]))
    }
...
First step is to transform the weighted matrix into a binary matrix. All 
weights `>= 0.25` will have a binary weight of 1. 
...
Apply now the function on the graph (for MCL group and Orthofinder orthogroup 
input).

```{r clique_og_mcl, cache = TRUE}
g <- igraph::graph_from_adjacency_matrix(bin_mat_clique_og_mcl, mode = "undirected", 
...
for (component_i in seq_along(cliques_greedy)) {
    ...
    for (clique_i in seq_along(cliques_greedy[[component_i]])) {
        ...
        ## find the rows of the members of the clique and write to membership_community
        ind_rows <- which(membership_community$X %in% cliques_greedy[[component_i]][[clique_i]])
...
First step is to transform the weighted matrix into a binary matrix. 
Set all edges that do not contain information from Orthofinder to 0, in other words
set the edges that are only derived from the MCL input to 0. 
...
bin_mat_clique_og[
    mat_type_cut %in% c("iadhore_mcl", "iadhore_mcl/mcscanx_mcl", "mcscanx_mcl")] <- 0
...
```{r export_net_cut}
write_graph(net_cut, file = "bin_mat_cut_graphml_manually_removed_edges.xml", format = "graphml")
```
...
if (all(names(V(net_cut)) == membership_community$X)) {
    ...
    for (i in seq_along(.components)) {
        ## define the color
        .color <- as.factor(membership_community[
            match(names(V(.components[[i]])), membership_community$X), "component_og_mcl"])

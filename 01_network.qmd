...
How many network components do we have now after manual removal of the false edges?
...
```{r add_information_on_orthogroup_mcl}
## add empty columns
...
Return the table with all the information (t-values, df, raw p-values, adjusted p-values) for the intercept and the `number_nodes` parameter (gradient).
...
What are the parameters that both show differences in the intercept and `number_nodes` parameter?
